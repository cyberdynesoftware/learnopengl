...
(defn create-vertex-buffer
  ...
  (let [buffer (BufferUtils/createFloatBuffer (* (.mNumVertices mesh) 8))]
    (dotimes [index (.mNumVertices mesh)]
      (let [vertex ^AIVector3D (.get (.mVertices mesh) index)
            normal ^AIVector3D (.get (.mNormals mesh) index)
            tex-coords ^AIVector3D (.get (.mTextureCoords mesh 0) index)]
        (.put buffer (.x vertex))
        (.put buffer (.y vertex))
        (.put buffer (.z vertex))
        (.put buffer (.x normal))
...
(defn create-index-buffer
  ...
  (let [buffer (BufferUtils/createIntBuffer (* (.mNumFaces mesh) 3))
        faces (.mFaces mesh)]
    (doseq [^AIFace face (take (.mNumFaces mesh) (repeatedly #(.get faces)))]
      (assert (= (.mNumIndices face) 3))
      (let [indices ^IntBuffer (.mIndices face)]
        (doseq [^int index (take (.mNumIndices face) (repeatedly #(.get indices)))]
          (.put buffer index))))
    (.flip buffer)))
...
(defn read-texture-name
  [material-indices ^AIScene scene]
  (assert (= (count material-indices) 1)
          "single material index expected")
  (let [material-pointer (.get (.mMaterials scene) ^int (first material-indices))
